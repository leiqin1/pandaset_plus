...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = ['car', 'truck', 'bus', 'bicycle', 'person']
    MIN_HEIGHT = [115, 72, 40]
    MAX_OCCLUSION = [0, 1, 2]
...
def image_box_overlap(boxes, query_boxes, criterion=-1):
    ...
    for k in range(K):
        qbox_area = (abs(query_boxes[k, 2] - query_boxes[k, 0]) *
                     abs(query_boxes[k, 3] - query_boxes[k, 1]))
        for n in range(N):
...
def kitti_eval(gt_annos,
    ...
    overlap_0_7 = np.array([[0.7, 0.7, 0.7, 0.5,
                             0.5], [0.7, 0.7, 0.7, 0.5, 0.5],
                            [0.7, 0.7, 0.7, 0.5, 0.5]])
    overlap_0_5 = np.array([[0.7, 0.7, 0.7, 0.5, 0.5],
    ...
    # min_overlaps = np.array([np.ones((6,len(current_classes)))*iou])
    class_to_name = {
        0: 'Car',
        1: 'Truck',
        2: 'Bus',
    ...
    # class_to_name = {0: 'Car', 1: 'Van', 2: 'Cyclist', 3: 'Pedestrian', 4: 'Person_sitting'}
    ...
    mAP11_bbox, mAP11_bev, mAP11_3d, mAP11_aos, mAP40_bbox, mAP40_bev, \
        mAP40_3d, mAP40_aos = do_eval_mm(gt_annos, dt_annos,
